...
<div class="container-fluid py-4">
    ...
    <?php $row = 0; $class=NULL;?>
    @foreach ($classMember as $item)
        @if ($item->studentID == Auth::user()->id)
            <?php $class = $item->classID?>
        @endif
    @endforeach
    @foreach ($classCourse as $item)
        @if ($row == 0)
            <div class="row">
        @endif
        ...
        <div class="card">
            ...
            <a href="{{url('openCourse/'.$item->id_course.'/'.$class)}}"><h5 class="mb-0"><i class="fas fa-door-open"></i> Open Course</h5></a>
            </div>
        </div>
        </div>
        @endif
