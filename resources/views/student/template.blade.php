    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
      <ul class="navbar-nav">
        <li class="nav-item z-20">
          <a class="nav-link {{(Request::segment(2) === NULL) ? '' : 'active'}}" href="{{route('student')}}">
        ...
        <li class="nav-item z-20 d-flex align-items-center justify-content-center mt-3">
          <?php
          ...
          @foreach ($attend as $item)
              @if (date('Y-m-d', strtotime($item->time)) == $today)
                  <?php $found = true?>
              @endif
          @endforeach
          @if (($hour == '8') || ($hour == '20') && (!$found)) 
            <a class="icon-move-right btn btn-primary" href="{{url('checkIn/'.Auth::user()->id)}}">Check-in<i class="fas fa-check-double"></i></a>
          @else 
            <button class="icon-move-right btn btn-primary" disabled>Check-in<i class="fas fa-check-double"></i></button>
          @endif
        </li>
      </ul>
    </div>
